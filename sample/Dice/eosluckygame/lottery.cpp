...
namespace eoswin {
    ...
    void lottery::endround() {
        ...
        // eosio_assert(rnd.lucky_player > 0, "has no lucky player");
        ...
        //to lucky player
        // auto to_lucky = rnd.reward_bucket * (100 - _global_state.token_bonus - _global_state.team_fee - _global_state.drawer_fee) / 100;
        auto to_lucky = rnd.reward_bucket * lucky_fee / 100;

        //to team
        auto to_team  = rnd.reward_bucket * team_fee / 100;

        //to token bonus bucket
        auto to_tokener = rnd.reward_bucket - to_team - to_lucky  - to_drawer;

        // INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {_self,N(active)}, {_self, _global_state.token_account, to_tokener, "[eos.win] Token holder bonus"} );
    ...
    bill_item lottery::buykeys(account_name buyer, asset quantity, string memo) {
        ...
        _global_state.total_users += 1;  // for tatal_bills
    ...
    void lottery::claimbonus(account_name actor, account_name owner) {
        ...
        // asset to_bonus = asset(bonus_bucket.amount * ((double)b.amount / supply.amount), bonus_bucket.symbol);
        ...
        auto to_tp_bonus = divide_token(owner, TP_CONTRACT, TP_SYMBOL, percent);
        ...
        // INLINE_ACTION_SENDER(eosio::token, reclaim)( _global_state.token_account, {_global_state.token_account, N(active)}, {owner, fee, "[eos.win] Fee for claim token bonus"} );
        ...
        _dividends.emplace(_self, [&](auto& info) {
            ...
            if (to_tp_bonus.amount > 0) {
                info.bonus.push_back(to_tp_bonus);
            }
    ...
    asset lottery::divide_token(account_name to, account_name token_account, symbol_name sym, const double percent) {
        ...
        eosio::token tokener(token_account);
        symbol_name sname = symbol_type(sym).name();
        uint64_t dividend_bucket = tokener.get_balance_amount(DIVI_ACCOUNT, sname);

        asset to_dividend = asset(dividend_bucket * percent, sym);
